...
namespace BankingTCPIPLib.Banking_System
{
    public class BankingDirector
    {
        ...
        public void LoginUser(string name, string password, string id)
        {
            ...
            if (account == null)
                throw new OperationCanceledException($"User with id {id} doesn't exist, wrong password or name!");
        ...
        public void WithdrawalFromAccount(decimal amount, string id)
        {
            ...
            WriteDb();
            Console.WriteLine(
                $"User with id: {account.BankAccount.UniqueId} withdrawal money fromm account, money amount: {amount}");
        }
